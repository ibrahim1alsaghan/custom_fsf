	...
	for instructor in instructors:
		instructor_details.append(
			frappe.db.get_value(
				"User",
				instructor,
				["name", "username", "full_name", "user_image", "first_name", "email"],
				as_dict=True,
			)
		)
...
def get_discussion_topics(doctype, docname, single_thread):
	...
	for topic in topics:
		...
		# Reply count
		topic["reply_count"] = frappe.db.count("Discussion Reply", filters={"topic": topic.name})
...
@frappe.whitelist()
def send_global_notification(subject):
	if "System Manager" not in frappe.get_roles(frappe.session.user):
		frappe.throw("Only System Managers can send announcements.")
	...
	make_notification_logs(notification, usernames)
	return "Announcement sent to all users"
